...
from immunity_network_topology.tests.utils import CreateGraphObjectsMixin
...
class TestMonitoringIntegration(Base, TransactionTestCase):
    @mock.patch(
        'immunity_monitoring.device.apps.DeviceMonitoringConfig.connect_device_signals'
    )
    @mock.patch('immunity_monitoring.db.timeseries_db.create_database')
    @mock.patch('immunity_monitoring.device.utils.manage_short_retention_policy')
    @mock.patch('immunity_monitoring.device.utils.manage_default_retention_policy')
    @mock.patch('immunity_monitoring.device.exportable._exportable_fields', [])
    @mock.patch('immunity_notifications.types.NOTIFICATION_TYPES', dict())
...
class TestAdmin(Base, TransactionTestCase):
    module = 'immunity_network_topology'
    app_label = 'topology'
    topology_model = Topology
    link_model = Link
    node_model = Node
